...
# NOTE: See 1281_3...I think I have done the next step incorrectly.  This person was admitted at 9:40 and 13:36 on the same day, and has
# insulin discontinuations of 12:30 and 13:30.  I used the last record which would mean that insulin was discontinued 6 minutes prior to admission.
# need to check with Becky.  If this is OK, then give her the #s per group
...
View(merged[check])
View(merged[merged$min_admit_insdc<0,check])
# I think the above code is working, but there are some people with insulin d/c before admission, and some people with insulin d/c 3+ days after admission
# need to check with Becky
...
merged$min_admit_glargine <- (as.duration(temp) +   ddays(merged$Glargine_mod_Date-merged$Admit_mod_Date))/dminutes(1)
...
merged <- merged[order(merged$StudyID.glucose,desc(merged$rebound_hyper)),]

# below doesn't work any more???  Should work, now that I am just subtracting dates
...
rebound_hyper[rebound_hyper$rebound_hyper==0,
             c("StudyID.glucose","min_admit_insdc","Glucose.from.Admit..Minutes.","min_insdc_gluc","Glucose","rebound_hyper","October.1.2014.Flag")]
...
N <- nrow(distinct(rebound_hyper,StudyID.glucose))
...
label(rebound_hyper$October.1.2014.Flag)='Study Period'
...
tab1 <- final_table(rebound_hyper,c('Sex','Ethnicity',"Insnew","Age","Height","Weight","HbA1c.Closest.to.Admisson","Median.Income"),
                    group=rebound_hyper$October.1.2014.Flag,margin=2,single=0,ron=2)
...
#rslt <- chisq.test(tbl)
tab_primary <- final_table(rebound_hyper,'rebound_hyper',group=rebound_hyper$October.1.2014.Flag,margin=2,single=0,ron=2)

####################################################################################################
#Median glucose #
####################################################################################################
tab_medgluc <- final_table(rebound_hyper,'median_glucose',group=rebound_hyper$October.1.2014.Flag,margin=2,single=0,ron=2)

```

# Background

The purpose of this study was to determine if early glargine administration in children with DKA prevents rebound hyperglycemia and recurrent ketogenesis without increasing the rate of hypoglycemia.  Participants who received glargine at onset of DKA management (i.e., those admitted after October 1, 2014) will be compared to those who received glargine at the time of discontinuation of IV insulin (those admitted prior to October 1, 2014).  
...
There was no difference in the rate of rebound hyperglycemia in the two groups (Table 2).
