...
hypo12 <- merged[order(merged$StudyID.glucose,desc(merged$hypo_12hr)),]
hypo12 <- hypo12 %>% group_by(StudyID.glucose) %>% filter(row_number(StudyID.glucose) == 1)
hypo12 <- merged %>% group_by(StudyID.glucose) %>% top_n(1,StudyID.glucose)
...
# keep unique admissions
rebound_hyper_distinct <- distinct(rebound_hyper,StudyID,.keep_all = TRUE)
N <- sum(!is.na(rebound_hyper_distinct$group))
Nexcl <- sum(is.na(rebound_hyper_distinct$group))

tab1 <- final_table(rebound_hyper_distinct,c('Sex','Ethnicity',"Insnew","Age","Height","Weight","HbA1c.Closest.to.Admisson","Median.Income"),
                    group=rebound_hyper_distinct$group,margin=2,single=0,ron=2)
...
#rslt <- chisq.test(tbl)
tab_primary <- final_table(rebound_hyper_distinct,'rebound_hyper',group=rebound_hyper_distinct$group,margin=2,single=0,ron=2)

####################################################################################################
#Median glucose #
####################################################################################################
tab_medgluc <- final_table(rebound_hyper_distinct,'median_glucose',group=rebound_hyper_distinct$group,margin=2,single=0,ron=2)

####################################################################################################
#Hypoglycemia within 12 hours after insulin discontinuation #
####################################################################################################
tab_hypo12 <- final_table(rebound_hyper_distinct,'rebound_hyper',group=rebound_hyper_distinct$group,margin=2,single=0,ron=2)
